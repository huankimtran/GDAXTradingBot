...
class OrderBookFeed(WebsocketClient):
    def __init__(self, marketName='BTC-USD'):
        super(OrderBookFeed, self).__init__(products=marketName)
    ...
    def on_message(self, message):
        ...
        if self._sequence == -1:
            self._asks = RBTree()
            self._bids = RBTree()
            res = self._client.get_product_order_book(self.MarketName,level=3)
            for bid in res['bids']:
                self.add({
                    'id': bid[2],
                    'side': 'buy',
